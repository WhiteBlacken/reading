...
from tools import format_gaze, generate_fixations, generate_pic_by_base64, show_fixations, get_word_location, \
    ...
    get_cnn_feature, get_row, get_euclid_distance, normalize_list, multiply_and_sum_lists, detect_fixations
import cv2


# Create your views here.

def get_fix_adjust_dataset(request):
    """
    需要的是：
    1. 单词序列 word_list
    2. 原始眼动序列 origin_fixations
    3. 调整后的眼动序列 adjust_fixations
    4. 页面标识 page_id
    """
    cnt = 0
    exp_list = get_available_experiment_id()
    for page_data in PageData.objects.filter(experiment_id__in=exp_list):
        try:
            ...
            df = pd.DataFrame(
                {
                    ...
                    "origin_fixations": [str([(int(x[0]), int(x[1])) for x in origin_fixations])],
                    "adjust_fixations": [str([(int(x[0]), int(x[1])) for x in result_fixations])]
                }
            )
            filename = "adjust_dataset.csv"
...
def get_all_time_pic(request):
    ...
    for page_data in page_data_ls[:1]:
        ...
        for i in range(1, 11):
            ...
            result_fixations[i][0] += x
            result_fixations[i][1] += y
        ...
        result_fixations[129][0] -= 30
        result_fixations[129][1] -= 30
        result_fixations[130][0] += 70
        result_fixations[130][1] -= 50

        result_fixations[133][1] -= 40
        result_fixations[134][1] -= 30

        for i, fix in enumerate(result_fixations):
            if fix[0] < 353:
                result_fixations[i][0] += random.randint(50, 100)
            result_fixations[i][0] += random.randint(-20, 20)
            result_fixations[1][0] += random.randint(-20, 20)


        fix_img = show_fixations(result_fixations, f"{path}fix-adjust.png", (0, 0, 255), label=100000)
        cv2.imwrite(f"{path}fix-adjust-2.png", fix_img)
...
def dataset_of_timestamp(request):
    """按照时间切割数据集"""
    filename = "exps/native.txt"
    file = open(filename, 'r')
    lines = file.readlines()

    experiment_list_select = list(lines)
    file = open("exps/exp.txt", 'r')
    lines = file.readlines()
    experiment_list_select.extend(list(lines))
...
def dataset_of_all_time(request):
    ...
    lines2 = file.readlines()
    experiment_list_select.extend(list(lines2))
    experiment_list_select = list(set(experiment_list_select))
    ...
    for experiment in experiments:
        ...
        success += 1
        logger.info(f"成功生成{success}条,失败{fail}条")
        print(f"成功生成{success}条,失败{fail}条")
        # except Exception:
        #     fail += 1
...
def get_available_experiment_id():
    filename = "exps/data1.txt"
    file = open(filename, 'r')
    lines = file.readlines()
