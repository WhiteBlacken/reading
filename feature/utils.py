...
def textarea(locations: list, danger_r=10) -> tuple:
    """
    确定文本区域的边界
    确定每一行的边界
    """
    ...
    for i, loc in enumerate(locations):
        ...
        if loc["top"] != pre_top:
            ...
            range_x = [row["left"] + (1 / 5) * row_length, row["right"] - (1 / 5) * row_length]
        ...
        if i == len(locations) - 1:
            # 最后一行不可能发生换行
            row = {"left": begin_left, "top": loc["top"], "right": loc["right"], "bottom": loc["bottom"]}
            rows.append(row)
    border = {
        "left": rows[0]["left"],
        "top": rows[0]["top"],
    ...
    return border, rows, danger_zone
...
def eye_gaze_to_feature(gaze, word_list, sentence_list, words_location, begin, pre_word_index):
    """
...
if __name__ == "__main__":
    ...
    border, rows, danger_zone = textarea(location)
    print(border)
    print(rows)
    print(danger_zone)

    assert in_danger_zone(700, 140, danger_zone) is True
    assert in_danger_zone(700, 210, danger_zone) is False
    assert in_danger_zone(500, 140, danger_zone) is False
